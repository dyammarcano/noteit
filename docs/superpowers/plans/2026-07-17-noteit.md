...
**Tech Stack:** Rust 1.96.0 / edition 2024, gix 0.85.0, rusqlite 0.40.1 (bundled), thiserror 2.0.18, tempfile 3.27.0.

**No clap.** The ambiguity rule (unknown first arg = free text that may contain `--`-like words) fights clap's parser, so Task 10 hand-rolls ~80 lines of parsing. `--help` is a static string. Do not add clap.

**No duplicated helpers.** `now()` and the row-mapping functions live in ONE place (`store/mod.rs` and `store/contexts.rs` respectively) and are shared via `pub(crate)`. Do not copy them between modules.
...
- **Errors are enums, never fail-open-to-`None`.** This is a deliberate divergence from a prior internal project's reference implementation, whose `Option` return collapses every failure into one indistinguishable case. noteit's ladder needs to tell them apart.
...
Observed on `/repos/noteit`: `roots=["27eed0bab50b27bbc271b4d9f09ccd89d09bd76c"]`, matching `git rev-list --max-parents=0 HEAD` exactly.
...
# No clap: the ambiguity rule fights its parser -- see Global Constraints.
[dependencies]
gix = "0.85"
...
fn upsert_context_is_idempotent_on_kind_and_key() {
    ...
    let a = store
        .upsert_context(Kind::Repo, "urn:noteit:v1:abc", "noteit", "/repos/noteit")
        .unwrap();
    let b = store
        .upsert_context(Kind::Repo, "urn:noteit:v1:abc", "noteit", "/repos/noteit")
        .unwrap();
...
fn path_contexts_under_finds_descendants_only() {
    let store = Store::open_in_memory().unwrap();
    store.upsert_context(Kind::Path, "/repos/noteit", "noteit", "/repos/noteit").unwrap();
    store.upsert_context(Kind::Path, "/repos/noteit/src", "src", "/repos/noteit/src").unwrap();
    store.upsert_context(Kind::Path, "/repos/other", "other", "/repos/other").unwrap();

    let found = store.path_contexts_under("/repos/noteit").unwrap();
    let keys: Vec<&str> = found.iter().map(|c| c.key.as_str()).collect();
    assert_eq!(keys.len(), 2, "got {keys:?}");
    assert!(keys.contains(&"/repos/noteit"));
    assert!(keys.contains(&"/repos/noteit/src"));
    assert!(!keys.contains(&"/repos/other"));
}
```
...
The `LIKE ... ESCAPE` prefix match is what keeps `/repos/other` out while catching `/repos/noteit/src`. If `path_contexts_under_finds_descendants_only` fails, print the generated `prefix` and check the backslash escaping first.
...
**Interfaces:**
- Consumes: `Store`, `StoreError`, `Context`, `Kind`, and the shared `store::now()`, `store::contexts::{row_to_context, CTX_COLS}` — reuse these, do not redeclare.
- Produces:
  - `pub struct Note { pub id: i64, pub context_id: i64, pub subpath: String, pub body: String, pub status: Status, pub created_at: i64, pub updated_at: i64 }` — derives `Debug, Clone, PartialEq, Eq` on `Status` (Task 10's tests require `PartialEq`)
...
fn search_finds_by_body_and_scopes_by_context() {
    ...
    let b = store
        .upsert_context(Kind::Repo, "urn:noteit:v1:def", "other", "/repos/other")
        .unwrap()
        .id;
...
Note: parsing is hand-rolled by design, not for lack of a library. The
ambiguity rule means an unknown first arg is free text that may itself
contain `--`-like words — which fights clap's parser rather than using it.
There is no clap dependency; see Global Constraints.
...
$exe = '/repos/noteit/target/debug/noteit.exe'
